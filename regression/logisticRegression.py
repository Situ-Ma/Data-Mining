...
def compute_avglogL(X,y,beta):
    eps = 1e-50
    n = y.shape[0] # number of labels
    ########## Please Fill Missing Lines Here ##########
    sig = sigmoid(np.dot(X,beta))
    logL = y * np.log(sig + eps) + (1 - y) * np.log((1 - sig) + eps)
    avglogL = np.sum(logL) / n
    return avglogL
...
def getBeta_BatchGradient(train_x, train_y, lr, num_iter, verbose):
    ...
    for iter in range(0, num_iter):
        ...
        deriv = np.zeros(p)
        for i in range(n):
            i_beta = np.exp(np.dot(beta.transpose(),train_x[i]))
            if i_beta == float('+inf'):
                p_ibeta = 1
            else:
                p_ibeta = i_beta / (1 + i_beta)
            for j in range(p):
                deriv[j] += train_x[i][j] * (train_y[i] - p_ibeta)
        beta += deriv.dot(lr)
    ...
    for iter in range(num_iter):
        ...
        for i in range(n):
            ...
            if i_beta == float('+inf'):
                ...
                p_ibeta = i_beta / (1 + i_beta)
            for j in range(p):
                #deri_1st[j] += train_x[i][j] * (train_y[i] - p_ibeta)
                #beta[j] += train_x[i][j] * (train_y[i] - p_ibeta) * lr
                for k in range(p):
                    # it is a p*p matrix
                    mat[j][k] -= train_x[i][j] * train_x[i][k] * p_ibeta * (1 - p_ibeta)

        #calculate right part: first 
        deriv = np.zeros(p)
        for i in range(n):
            i_beta = np.exp(np.dot(beta.transpose(),train_x[i]))
            if i_beta == float('+inf'):
        ...
        if np.linalg.det(mat) == 0.0:
            mat += np.identity(p)
        beta -= np.dot(np.linalg.inv(mat),deriv)
...
def getBeta_regularization(train_x, train_y, lr, num_iter, verbose):
    beta = np.random.rand(train_x.shape[1])
...
class LogisticRegression(object):
    ...
    def train(self, algType):
        ...
        if(algType == '0'):
            ...
            beta = getBeta_regularization(newTrain_x, self.train_y.values, self.lr, self.num_iter, self.verbose)
            print('Beta: ', beta)
        else:
