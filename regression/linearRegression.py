...
def getBeta(train_x, train_y):   
    ...
    left = np.linalg.inv(np.dot(xT,train_x))
    right = np.dot(xT,train_y)
...
def getBetaBatchGradient(train_x, train_y, lr, num_iter):
    ...
    for iter in range(0, num_iter):
        ...
        for i in range(n):
           ########## Please Fill Missing Lines Here ##########
           err = np.dot(train_x[i].transpose(),beta) - train_y[i]
           deriv += err * train_x[i]
        deriv = deriv/n
...
def getBetaStochasticGradient(train_x, train_y, lr):
    ...
    for iter in range(epoch):
        ...
        for i in range(n):
            idx = indices[i]
            ########## Please Fill Missing Lines Here ##########
            err = train_y[idx] - np.dot(train_x[idx].transpose(),beta)
            err *= train_x[idx]
            beta += err * lr
    return beta
...
class LinearRegression(object):
    ...
    def __init__(self,lr=0.003, num_iter=1000):
        self.lr = lr
