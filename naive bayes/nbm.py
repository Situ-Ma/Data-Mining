...
class NB_model():
    ...
    def fit(self, train_data, train_label, vocab, if_use_smooth=True):
        ...
        p_word = train_data.groupby(['classIdx','wordIdx'])
        p_class = train_data.groupby(['classIdx'])
        denom = p_class['count'].sum()
        numer = p_word['count'].sum()
        if if_use_smooth:
            p = (numer + 1) / (denom + self.num_vocab) 
        else:
            p = numer / denom
        p = p.unstack()

        for classID in range(1,self.num_classes+1):
        	p.loc[classID,:] = p.loc[classID,:].fillna(float(1)/(p_class['count'].sum()[classID] + self.num_vocab))

        self.Pr_dict = p.to_dict()
        ...
        for docIdx in range(1, len(new_dict)+1):
            ...
            for classIdx in range(1,self.num_classes+1):
                ...
                for wordIdx in new_dict[docIdx]:
                	try:
	                	p = self.Pr_dict[wordIdx][classIdx]
	                	score_dict[classIdx] += new_dict[docIdx][wordIdx]*np.log(p)
	                except:
	                	p = 0
    ...
    def get_prior_prob(self,train_label, verbose=True):
        ...
        for c in unique_class:
            ...
            prob = train_label.count(c) / total
            self.pi[c] = prob
            # ============================================================
        if verbose:
