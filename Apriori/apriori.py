...
def get_items_with_min_support(item_set, all_transactions, min_support,
    ...
    for item, count in local_set.items():
        #################### TODO
        #################### TODO
        if get_support(set_count_map,item,N) >= min_support:
            rtn.add(item)
    ...
    for key, value in freq_map.items()[1:]:
        for item in value:
            ...
            for element in _subsets:
                ...
                if len(remain) > 0:
                    confidence = get_support(set_count_map, item, N)/get_support(set_count_map,element,N)#################### TODO
